...
BSBI_Search::BSBI_Search(const std::string& main)
{
	...
	if( !conf ) err(main), err(), throw std::exception(); // TODO: change it
	...
	if( !w2ip ) err(word2index_pos_filename), err(), throw std::exception();
	std::string s;
	int pos;
	while( w2ip >> s >> pos )
	...
	// can be improve
	if( !file_lst ) err(files_filename), err(), throw std::exception();
	...
	for( ; it != v[0].end(); it++)
	{
		...
		for(int i = 1; i < (int) v.size(); i++)
		{
			...
			if(f == v[i].end() ||
				...
				//&& *f != Term(v[i].begin()->word, it->file, pos + 2))) // FIXME: dirty trick
			{ 
				found = false;  
				break; 
			}
			else if( *f == Term(v[i].begin()->word, it->file, pos + 1) ) pos++;
			//else if( *f == Term(v[i].begin()->word, it->file, pos + 2) ) pos += 2; // case of double punctuation
		}
		if( found )
		{
